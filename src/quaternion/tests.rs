...
fn approx_eq_abs_surface() {
    ...
    assert_approx_eql_abs_tol!(q, minus_q, 1e-6);
    assert!(!approx_eql_abs_tol!(q, far, 1e-6));
    assert_approx_eql_abs!(q, minus_q);
...
fn approx_eq_rel_surface() {
    ...
    assert_approx_eql_rel_tol!(q, minus_q, 1e-4);
    assert!(!approx_eql_rel_tol!(q, far, 1e-4));
    assert_approx_eql_rel!(q, minus_q);
...
fn inverse_surface() {
    ...
    assert_approx_eql_abs!(inverse, trait_inverse);
    assert_approx_eql_abs!(inverse, expected);
    assert_approx_eql_abs!(trait_inverse, expected);
}
...
fn compose_surface() {
    ...
    assert_approx_eql_abs!(composed_inherent, composed_op);
    assert_approx_eql_abs!(composed_inherent, expected);
}
...
fn rotate_vector_surface() {
    ...
    assert_approx_eql_abs_tol!(rotated_inherent, rotated_op, 1e-6);
    assert_approx_eql_abs_tol!(rotated_inherent, expected, 1e-6);
}
...
fn axis_angle_surface() {
    ...
    let qz_180 = Quaternion::from_array([0.0_f32, 0.0, 1.0, 0.0]);
    assert_approx_eql_abs_tol!(q_from, qz_180, 1e-6);

    let (axis, angle) = qz_180.to_axis_angle();

    assert_approx_eql_abs_tol!(axis, Vector::from_array([0.0_f32, 0.0, 1.0]), 1e-6);
    assert_approx_eql_abs_tol!(angle, core::f32::consts::PI, 1e-6);

    let reconstructed = Quaternion::from_axis_angle(axis, angle);
    assert_approx_eql_abs_tol!(reconstructed, qz_180, 1e-6);

    let identity = Quaternion::from_array([0.0_f32, 0.0, 0.0, 1.0]);
    let (identity_axis, identity_angle) = identity.to_axis_angle();

    assert_approx_eql_abs_tol!(identity_axis, Vector::from_array([1.0_f32, 0.0, 0.0]), 1e-6);
    assert_approx_eql_abs_tol!(identity_angle, 0.0_f32, 1e-6);
}
...
fn interpolation_surface() {
    ...
    assert_approx_eql_abs_tol!(lerp_inherent, lerp_trait, 1e-6);
    assert_approx_eql_abs_tol!(lerp_inherent, lerp_expected, 1e-6);

    let nlerp_half = id.nlerp(z_180, 0.5);
    let nlerp_half_neg_rhs = id.nlerp(-z_180, 0.5);

    assert_approx_eql_abs_tol!(nlerp_half.length(), 1.0, 1e-6);
    assert_approx_eql_abs_tol!(nlerp_half, half_turn, 1e-6);
    assert_approx_eql_abs_tol!(nlerp_half_neg_rhs, half_turn_neg_branch, 1e-6);

    let slerp_half = id.slerp(z_180, 0.5);
    let slerp_half_neg_rhs = id.slerp(-z_180, 0.5);

    assert_approx_eql_abs_tol!(slerp_half.length(), 1.0, 1e-6);
    assert_approx_eql_abs_tol!(slerp_half, half_turn, 1e-6);
    assert_approx_eql_abs_tol!(slerp_half_neg_rhs, half_turn_neg_branch, 1e-6);
}
...
fn length_and_normalize_surface() {
    ...
    assert_approx_eql_abs_tol!(q.length_squared(), 25.0, 1e-6);
    assert_approx_eql_abs_tol!(<Quaternion<f32> as LengthSquared>::length_squared(q), 25.0, 1e-6);

    assert_approx_eql_abs_tol!(q.length(), 5.0, 1e-6);
    assert_approx_eql_abs_tol!(<Quaternion<f32> as Length>::length(q), 5.0, 1e-6);
    ...
    assert_approx_eql_abs_tol!(n, Quaternion::from_array([0.0, 0.0, 0.6, 0.8]), 1e-6);
    assert_approx_eql_abs_tol!(nt, Quaternion::from_array([0.0, 0.0, 0.6, 0.8]), 1e-6);
