...
fn approx_eq_abs_surface() {
    ...
    assert_approx_eq_abs!(a, b, 1e-6);
    assert_approx_ne_abs!(a, c, 1e-6);
...
fn approx_eq_rel_surface() {
    ...
    assert_approx_eq_rel!(a, b, 1e-4);
    assert_approx_ne_rel!(a, c, 1e-4);
...
fn length_and_normalize_surface() {
    ...
    assert_approx_eq_abs!(v.length_squared(), 25.0, 1e-6);
    assert_approx_eq_abs!(<Vector<f32, 2> as LengthSquared>::length_squared(v), 25.0, 1e-6);

    assert_approx_eq_abs!(v.length(), 5.0, 1e-6);
    assert_approx_eq_abs!(<Vector<f32, 2> as Length>::length(v), 5.0, 1e-6);
    ...
    assert_approx_eq_abs!(n, Vector::from_array([0.6, 0.8]), 1e-6);
    assert_approx_eq_abs!(nt, Vector::from_array([0.6, 0.8]), 1e-6);
...
fn lerp_surface() {
    ...
    assert_approx_eq_abs!(inherent, expected, 1e-6);
    assert_approx_eq_abs!(trait_path, expected, 1e-6);
    assert_approx_eq_abs!(inherent, trait_path, 1e-6);
}
...
fn minmax_surface() {
    ...
    assert_approx_eq_abs!(min_f, Vector::<f32, 2>::from_array([3.3, 2.2]), 1e-6);
    assert_approx_eq_abs!(max_f, Vector::<f32, 2>::from_array([5.5, 7.7]), 1e-6);
}
...
fn abs_surface() {
    ...
    let abs_f = nf.abs();
    assert_approx_eq_abs!(abs_f, Vector::<f32, 2>::from_array([1.5, 2.5]), 1e-6);
}
...
fn cross_product_surface() {
    ...
    let cross = a.cross(b);
    assert_approx_eq_abs!(cross, Vec3::new(-3.0, 6.0, -3.0), 1e-6);
    ...
    assert_approx_eq_abs!(x.cross(y), z, 1e-6);
    assert_approx_eq_abs!(y.cross(z), x, 1e-6);
    assert_approx_eq_abs!(z.cross(x), y, 1e-6);

    // Anti-commutativity: a × b = -(b × a)
    let result1 = a.cross(b);
    let result2 = b.cross(a);
    assert_approx_eq_abs!(result1, -result2, 1e-6);
}
